...
class ReadNormalNewsCell: NewsCell {
    ...
    @IBOutlet weak var icon2recommendConstraint: NSLayoutConstraint!
    ...
    func setupSubView() {
        ...
        if let _ = readNewsModel.recReason {
            ...
            self.icon2recommendConstraint.priority = 999
        }else {
            self.recommendView.hidden = true
            self.icon2recommendConstraint.priority = 500
        }
